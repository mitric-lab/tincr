...
pub fn get_h2o_molecule() -> System {
    let config = get_config();
    let path_prefix = get_path_prefix();
    let filename: String = format!(
        "{}/tests/data/h2o/h2o.xyz",
        path_prefix
    );
    let system: System = System::from((filename.as_str(), config));
    return system;
}
//
// pub fn get_benzene_molecule() -> System {
