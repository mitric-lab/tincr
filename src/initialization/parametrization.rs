...
pub fn construct_gaussian_overlap(atomic_numbers: &[u8], positions: ArrayView2<f64>, hubbard_u: &HashMap<u8, f64>) -> (Array2<f64>) {
    let n_atoms: usize = atomic_numbers.len();
    let mut gauss_omega: Array2<f64> = Array::zeros((n_atoms, n_atoms));
    let mut sigma: Array1<f64> = Array::zeros(n_atoms);
    for (index, i) in atomic_numbers.iter().enumerate() {
        sigma[index] = 1.329 / (8.0 * 2.0_f64.log(E)) * 1.0 / hubbard_u[i];
    }
    for (i, pos_i) in positions.outer_iter().enumerate() {
        for (j, pos_j) in positions.outer_iter().enumerate() {
...
pub fn get_atomtypes(atomic_numbers: &[u8]) -> (HashMap<u8, String>, Vec<u8>) {
