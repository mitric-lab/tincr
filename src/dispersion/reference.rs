...
const MAX_NREF: usize = 7;
const MAX_CN: usize = 19;
const TRAPZD_POINTS: usize = 23;
...
pub fn get_refgw(atomic_number: u8) -> [usize; MAX_CN] {
    let mut cnc: [usize; MAX_CN + 1] = [0; MAX_CN + 1];
    cnc[0] = 1;
    let mut ngw: [usize; MAX_CN] = [0; MAX_CN];
...
pub fn get_refalpha(ga: f64, gc: f64, atomic_number: u8) -> [[f64; TRAPZD_POINTS]; MAX_NREF] {
    ...
    let ref_: usize = get_nref(atomic_number);
    for ir in 0..ref_ {
        let is = REFSYS[atomic_number as usize][ir];  // Atomic number in the reference system
        ...
        let aiw: [f64; TRAPZD_POINTS] = array_scalar_mult(SECAIW[is as usize],
                                                          SSCALE[is as usize] * zeta(
                                                              ...
                                                              get_hardness(is)*gc,
                                                              iz,
                                                              CLSH[atomic_number as usize][ir] + iz));
        // alpha(:, ir) = max(ascale(ir, num)*(alphaiw(:, ir, num) - hcount(ir, num)*aiw), 0.0_wp)
        alpha[ir] = array_scalar_comp(
            array_scalar_mult(
                array_substraction(
                    ALPHAIW[atomic_number][ir],
                    array_scalar_mult(aiw, HCOUNT[atomic_number][ir])
                ), ASCALE[atomic_number][ir]),
        0.0);
    }
    return alpha;
}
