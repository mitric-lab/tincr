...
impl System{
    ...
    pub fn ci_overlap(
        ...
        let s_mo:Array2<f64> = orbs_i.t().dot(&s_ao.dot(&old_orbs));
        ...
        let ci_coeff:ArrayView3<f64> = ci_coeff.t().into_shape([n_roots,n_occ,n_virt]).unwrap();
        let old_ci_coeff:ArrayView3<f64> = old_ci_coeff.t().into_shape([n_roots,n_occ,n_virt]).unwrap();
