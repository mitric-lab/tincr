...
pub fn fmo_gs_gradients(
    ...
    let grad_vec_total_dimers:Vec<Array1<f64>> = pair_results.par_iter().enumerate().filter_map(|(pair_index,pair)| if pair.energy_pair.is_some(){
        ...
        let dimer_gradient:Array1<f64> = dimer_gradient_e0 + dimer_gradient_vrep;
        ...
        grad_total_dimers = grad_total_dimers + embedding_gradient;
        Some(grad_total_dimers)
    }
    else{
        ...
        grad_total_dimers
            .slice_mut(s![3 * index_frag_a..3 * index_frag_a + 3 * atoms_a])
            .add_assign(&gradient_frag_a);
        grad_total_dimers
            .slice_mut(s![3 * index_frag_b..3 * index_frag_b + 3 * atoms_b])
            .add_assign(&gradient_frag_b);
        Some(grad_total_dimers)
    }).collect();
    ...
    // }
    let mut grad_total_dimers: Array1<f64> = Array1::zeros(grad_total_frags.raw_dim());
    for grad in grad_vec_total_dimers.iter(){
        grad_total_dimers = grad_total_dimers + grad;
    }
