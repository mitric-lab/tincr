...
pub fn fmo_fragments_gradients_ncc(
    ...
    'ncc_loop: for i in 0..max_iter {
        let monomer_iter: Vec<monomer_grad_result> = fragments
            ...
            .map(|(index, frag)| {
                ...
                let mut dq_diff: Array1<f64> = frag.final_charges.clone();
                let mut p_diff: Array2<f64> = frag.final_p_matrix.clone();
                if i != 0{
                    dq_diff = dq_diff - dq_old[index].clone();
                    p_diff = p_diff - pmat_old[index].clone();
                }
                let dq_rmsd_val:f64 = dq_diff.map(|x| x.abs()).max().unwrap().to_owned();
                let p_rmsd_val:f64 = p_diff.map(|val| val * val).mean().unwrap().sqrt();
                let monomer_grad_result:monomer_grad_result = monomer_grad_result::new(
                    frag.final_charges.clone(),
                    frag.final_p_matrix.clone(),
                    s,
                    om_monomer.clone(),
                    p_rmsd_val,
                    dq_rmsd_val,
                ...
                //     let dq_rmsd_val:f64 = dq_diff.map(|x| x.abs()).max().unwrap().to_owned();
                //     let p_rmsd_val:f64 = p_diff.map(|val| val * val).mean().unwrap().sqrt();
                //     // dq_rmsd[index] = dq_rmsd_val;
                //     // p_rmsd[index] = p_rmsd_val;
                //     let monomer_grad_result:monomer_grad_result = monomer_grad_result::new(
                //         frag.final_charges.clone(),
                //         frag.final_p_matrix.clone(),
                //         s,
                ...
                //         energy
                //     );
                //     //monomer_iter.push(monomer_grad_result);
        ...
        let mut energies_arr:Array1<f64> = Array1::zeros(length);
        for (index,item) in monomer_iter.iter().enumerate(){
            energies_arr[index] = item.energy;
            dq_rmsd[index] = item.dq_diff;
            p_rmsd[index] = item.p_rmsd;
            let frag_index: usize = frag_indices[index];
            let frag_atoms:usize = fragments[index].n_atoms;
            dq_arr.slice_mut(s![frag_index..frag_index + frag_atoms])
                .assign(&item.dq_old);

            if i == 0{
                dq_old.push(item.dq_old.clone());
                pmat_old.push(item.p_mat_old.clone());
                s_matrices.push(item.s_mat.clone());
                om_monomer_matrices.push(Array1::zeros(frag_atoms));
            }
            else{
                dq_old[index] = item.dq_old.clone();
                pmat_old[index] = item.p_mat_old.clone();
                om_monomer_matrices[index] = item.om_monomer.clone();
            }
        }
    ...
    let gradients_vec: Vec<frag_gradient_result> = fragments
        ...
        .map(|(index, frag)| {
            ...
            let (qtrans,qtrans_oo): (Array2<f64>,Array2<f64>) = fragment_trans_charges(
                &frag.atomic_numbers,
...
pub fn fmo_gradient_pairs_embedding_esdim(
    ...
    let response_contribution: Array1<f64> = response_contribution_gradient_new(
        fragments,
        indices_frags,
        g0_total,
        frag_gradient_results,
...
pub fn fmo_zvector_routine(
    ...
    let mut z_old: Vec<Array2<f64>> = fragments
        ...
        .map(|(ind, frag)| {
            ...
            // let mut a_mat_4d_loop: Array4<f64> = Array4::zeros((dim_virt, dim_occ, dim_virt, dim_occ));
            // for i in (0..dim_virt).into_iter() {
            //     for j in (0..dim_occ).into_iter() {
            //         for k in (0..dim_virt).into_iter() {
            //             for l in (0..dim_occ).into_iter() {
            //                 for a in (0..frag.n_atoms).into_iter(){
            //                     for b in (0..frag.n_atoms).into_iter(){
            //                         a_mat_4d_loop[[i,j,k,l]] += frag.g0[[a,b]] * qtrans_3d[[a,i,j]] * qtrans_3d[[b,k,l]];
            //                     }
            //                 }
            //             }
            //         }
            //     }
            // }
            // a_mat_4d_loop = -4.0 * a_mat_4d_loop;
            //
            // let mut a_mat_4d_loop_2: Array4<f64> = Array4::zeros((dim_virt, dim_occ, dim_virt, dim_occ));
            // for i in (0..dim_virt).into_iter() {
            //     for j in (0..dim_occ).into_iter() {
            //         for k in (0..dim_virt).into_iter() {
            //             for l in (0..dim_occ).into_iter() {
            //                 let mut esp_like: Array1<f64> = Array1::zeros(frag.n_atoms);
            //                 // for c in (0..frag.n_atoms).into_iter(){
            //                 //     esp_like = esp_like + frag.g0.slice(s![..,c]).to_owned() * qtrans_3d[[c,k,l]];
            //                 // }
            //                 let esp_like: Array1<f64> = frag.g0.dot(&qtrans_3d.slice(s![.., k, l]));
            //                 let mut esp_mat_ao: Array2<f64> = Array2::zeros(s_matrices[ind].raw_dim());
            //                 let mut mu: usize = 0;
            //                 let mut nu: usize;
            //                 for (a, z_i) in frag.atomic_numbers.iter().enumerate() {
            //                     for _ in &frag.calculator.valorbs[z_i] {
            //                         nu = 0;
            //                         for (b, z_j) in frag.atomic_numbers.iter().enumerate() {
            //                             for _ in &frag.calculator.valorbs[z_j] {
            //                                 esp_mat_ao[[mu, nu]] = (esp_like[a] + esp_like[b]);
            //                                 nu = nu + 1;
            //                             }
            //                         }
            //                         mu = mu + 1;
            //                     }
            //                 }
            //                 let s_esp_term: Array2<f64> = esp_mat_ao * s_matrices[ind].view();
            //                 a_mat_4d_loop_2[[i,j,k,l]] += orbs.slice(s![..,dim_occ+i]).t().dot(&s_esp_term).dot(&orbs.slice(s![..,j]));
            //             }
            //         }
            //     }
            // }
            // a_mat_4d_loop_2 = -2.0 * a_mat_4d_loop_2;
            ...
            //         let esp_mat_ao:Array2<f64> = &esp_arr.broadcast((esp_ao.len(), esp_ao.len())).unwrap() + &esp_ao;
            //         let s_esp_term: Array2<f64> = esp_mat_ao * s_matrices[ind].view();
            ...
            // assert_eq!(a_mat_4d_loop,a_mat_4d_loop_2,"A matrices NOT EQUAL!");
    ...
    'scz_loop: for iter in 0..max_iter {
        ...
        let lagrangian_new: Vec<Array2<f64>> = fragments
            ...
            .map(|(ind, frag)| {
                ...
                if converged[ind] == false {
                    ...
                    lagrangian = &initial_lagrangian[ind] - &sum;
                } else {
        ...
        let z_new: Vec<Array2<f64>> = fragments
            ...
            .map(|(ind, frag)| {
                ...
                if converged[ind] == false {
                    ...
                    //     .into_shape((dim_virt * dim_occ, dim_virt * dim_occ))
                    //     .unwrap();
                    let lagrangian_1d: Array1<f64> = lagrangian_new[ind]
                        .clone()
                        .into_shape(dim_virt * dim_occ)
                        .unwrap();
        ...
        let mut z_vec_rmsd:Vec<f64> = Vec::new();
        let mut lagrangian_vec_rmsd:Vec<f64> = Vec::new();
        // check convergence
        for (index, z_vec) in z_new.iter().enumerate() {
            let z_diff: Array2<f64> = z_vec - &z_old[index];
            let lagrangian_diff:Array2<f64> = &lagrangian_new[index] - &lagrangian_old[index];
            let z_rmsd: f64 = z_diff.map(|val| val * val).mean().unwrap().sqrt();
            let lagrangian_rmsd:f64 = lagrangian_diff.map(|val| val * val).mean().unwrap().sqrt();
            z_vec_rmsd.push(z_rmsd);
            lagrangian_vec_rmsd.push(lagrangian_rmsd);
        ...
        println!("Z rmsd {:?}",z_vec_rmsd);
        println!("lagrangian rmsd {:?}",lagrangian_vec_rmsd);
...
pub fn response_contribution_gradient_new(
    fragments: &Vec<Molecule>,
    indices_frags: &Vec<usize>,
    g0_total: ArrayView2<f64>,
    frag_gradient_results: &Vec<frag_gradient_result>,
    ...
    let frag_gradients: Vec<Array1<f64>> = fragments
        ...
        .map(|(ind, frag)| {
            ...
            let index_frag_i: usize = indices_frags[ind];
            let atoms_i:usize = frag.n_atoms;
            ...
            for (ind_k, frag_k) in fragments.iter().enumerate() {
                ...
                for a in (0..3 * frag_k.n_atoms).into_iter(){
                    let g0_deriv_q:Array1<f64> = g0.dot(&frag_gradient_results[ind_k].ws_pds.slice(s![a,..]));
                    let mut mu: usize = 0;
                    let mut q_deriv_ao: Array1<f64> = Array1::zeros(frag.calculator.n_orbs);
                    for (i, z_i) in frag.atomic_numbers.iter().enumerate() {
                        for _ in &frag.calculator.valorbs[z_i] {
                            q_deriv_ao[mu] = g0_deriv_q[i];
                            mu = mu + 1;
                        }
                    }
            ...
            for dir_xyz in (0..3).into_iter(){
                for a in (0.. frag.n_atoms){
                    let index:usize = 3*a+dir_xyz;
                    let mut b_mat: Array2<f64> = Array2::zeros((dim_virt, dim_occ));

                    // sum_c (g1_ac + g1_bc) * dq_c
                    // sum_c (g0_ac + g0_bc) * dq_c
                    let mut g1_temp:Array2<f64> = Array2::zeros((atoms_i,atoms_i));
                    g1_temp.slice_mut(s![a,..]).assign(&g1_2d.slice(s![3*index_frag_i+index,index_frag_i..index_frag_i+atoms_i]));
                    g1_temp.slice_mut(s![..,a]).assign(&g1_2d.slice(s![3*index_frag_i+index,index_frag_i..index_frag_i+atoms_i]));
                    let g1_dq:Array1<f64> = g1_temp.dot(&dq_vec[ind]);

                    // let mut g1_dq:Array1<f64> = Array1::zeros(atoms_i);
                    ...
                    for j in (0..dim_occ) {
                        ...
                        let h_s_term:Array2<f64> = h_grad_a.clone() - s_term;
                        let hs_ij:Array1<f64> = orbs_i.slice(s![..,dim_occ..]).t().dot(&h_s_term).dot(&orbs_i.slice(s![..,j]));

                        b_mat.slice_mut(s![..,j]).assign(&hs_ij);
                    }
                    gradient[index] = z_vectors[ind].t().dot(&b_mat).trace().unwrap();
                }
            }
    ...
    for (index, frag) in fragments.iter().enumerate() {
        response_gradient = response_gradient + frag_gradients[index].view();
    }
    ...
    let frag_gradients: Vec<Array1<f64>> = fragments
        ...
        .map(|(ind, frag)| {
            ...
            for a in (0..3 * frag.n_atoms).into_iter() {
                ...
                let charge_arr: Array2<f64> = charge_ao.clone().insert_axis(Axis(1));
                let charge_mat_ao:Array2<f64> = &charge_arr.broadcast((charge_ao.len(), charge_ao.len())).unwrap() + &charge_ao;
...
pub struct monomer_grad_result{
    dq_old:Array1<f64>,
    p_mat_old:Array2<f64>,
    s_mat:Array2<f64>,
    om_monomer:Array1<f64>,
    p_rmsd:f64,
    dq_diff:f64,
    energy:f64
}

impl monomer_grad_result{
    pub fn new(
        dq_old:Array1<f64>,
        p_mat_old:Array2<f64>,
        s_mat:Array2<f64>,
