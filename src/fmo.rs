...
pub fn fmo_numerical_gradient_4th_order(
    atomic_numbers: &Vec<u8>,
    positions: &Array1<f64>,
    config: GeneralConfig,
) -> Array1<f64> {
...
pub fn fmo_numerical_gradient_6th_order(
    atomic_numbers: &Vec<u8>,
    positions: &Array1<f64>,
    config: GeneralConfig,
) -> Array1<f64> {
    ...
    for ind in (0..positions.len()).into_iter() {
        ...
        let energy_5: f64 = numerical_gradient_routine(
            &atomic_numbers,
            positions,
            config.clone(),
            subgraph.clone(),
        ...
        let energy_6: f64 = numerical_gradient_routine(
            &atomic_numbers,
            positions,
            config.clone(),
            subgraph.clone(),
        ...
        let grad_temp: f64 = (energy_1 - 9.0*energy_2 +45.0*energy_3 -45.0*energy_4 + 9.0*energy_5 - energy_6) / (60.0 * h);
        gradient[ind] = grad_temp;
    }
    return gradient;
}
        ...
        let energies_vec: Vec<f64> = fragments
            ...
            .map(|(index, frag)| {
                ...
                if converged[index] == false{
                    ...
                    if i == 0 {
                        let energy_diff:f64 = (energy-old_energies[index]).abs();
                        old_energies[index] = energy;
                        ...
                        if energy_diff < conv && dq_rmsd[index] < conv && p_rmsd[index] < conv{
            ...
            for (ind2, molecule_b) in fragments.iter().enumerate() {
                ...
                if ind1 < ind2 {
                    ...
                    if (min_dist / vdw_radii_sum) < 2.0 {
                        ...
                        let embedding_a: f64 = ddq_arr
                            .slice(s![0..frag_a_atoms])
                            .dot(&(&om_monomers[ind1] - &g0_a_2.dot(&molecule_b.final_charges)));
                        let embedding_b: f64 = ddq_arr.slice(s![frag_a_atoms..]).dot(
