...
pub fn get_exc_energies(
    ...
    let r_lr = molecule.calculator.r_lr.unwrap_or(defaults::LONG_RANGE_RADIUS);
...
pub fn casida(
    ...
    let XmY_final:Array3<f64> = XmY.to_owned_f().t().into_shape((n_occ * n_virt, n_occ, n_virt)).unwrap().to_owned();
    let XpY_final:Array3<f64> = XpY.to_owned_f().t().into_shape((n_occ * n_virt, n_occ, n_virt)).unwrap().to_owned();
...
fn excited_energies_tda_routine() {
    ...
    let  omega_ref_out: Array1<f64> = array![
       0.5639270376740005, 0.6133146373942717, 0.6289193025552757,
       0.6699836563376497, 0.7241847548895822, 0.7826629904064589,
       1.0415454883803148, 1.0853598783803207
];

    let (omega_out, c_ij, XmY, XpY) =
        get_exc_energies(&f_occ.to_vec(), &mol, None, &S, &orbe, &orbs, Some(String::from("TDA")));
...
fn excited_energies_hermitian_davidson_routine() {
    ...
    let  omega_ref_out: Array1<f64> = array![
       0.6599613806976925, 0.7123456990588427, 0.7456810724193917,
       0.7930925652350208
];
    let  XmY_ref: Array3<f64> = array![
...
fn excited_energies_non_hermitian_davidson_routine() {
    ...
    let  omega_ref_out: Array1<f64> = array![
       0.5639270376740008, 0.6133146373942723, 0.6284386094838724,
       0.6699518396823421
];
    let  XmY_ref: Array3<f64> = array![
