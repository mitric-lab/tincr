...
pub fn gradient_nolc(
    ...
    let diff_d: ArrayView2<f64> = &d - &d_ref;
    // computing F(D-D0)
    ...
    for i in 0..(3 * molecule.n_atoms) {
        grad_e0[i] += grad_h0.slice(s![i, .., ..]).dot(&d);
        grad_e0[i] += 0.5 * fdmd0.slice(s![i, .., ..]).dot(&diff_d);
        grad_e0[i] -= grad_s.slice(s![i, .., ..]).dot(&d_en);
    }
...
fn f_v(
    ...
    g0_a0: ArrayView2<f64>,
    g1_a0: ArrayView3<f64>,
    n_atoms: usize,
    n_orb: usize,
) -> Array3<f64> {
    let mut f: Array3<f64> = Array3::zeros([3 * n_atoms, n_orb, n_orb]);
    let vp: Array2<f64> = v + v.t();
    let sv: Array2<f64> = (s * &vp).sum_axis(Axis(1));
    let gsv = g0_a0.dot(&sv);
    ...
    for nc in 0..(3 * n_atoms) {
        gdsv.slice_mut(s![nc, ..]).assign(
            g0_a0
                .dot((grad_s.slice(s![nc, .., ..]) * &vp))
                .sum_axis(Axis(1)),
        );
        dgsv.slice_mut(s![nc, ..])
            .assign(g1_a0.slice(s![nc, .., ..]).dot(&sv));
    }
    // a and b are AOs
    for a in 0..n_orb {
        for b in 0..n_orb {
            f.slice_mut(s![.., a, b]).assign(
                grad_s.slice(s![.., a, b]) * (gsv[a] + gsv[b])
                    + s[[a, b]] * (dgsv.slice(s![.., a]) + dgsv.slice(s![.., b]))
                    + s[[a, b]] * (gdsv.slice(s![.., a]) + gdsv.slice(s![.., b])),
            );
        }
    }
    f *= 0.25;
    ...
    for (i, z_i) in atomic_numbers.iter().enumerate() {
        ...
        for (j, z_j) in atomic_numbers.iter().enumerate() {
            if i != j {
                ...
                let v_ij_deriv: f64 = v_rep[&(z_1, z_2)].spline_deriv(r);
                grad_i += directions.slice(s![i, j, ..]) * v_ij_deriv;
            }
        }
