use ndarray::{Array, Array1, ArrayView1, array, ArrayView2};
use std::cmp::{max, min};
...
fn fermi_occupation_t0(orbe: ArrayView1<f64>, n_elec_paired: usize, n_elec_unpaired: usize) -> (f64, Vec<f64>){
...
fn fermi_occ_at_t0() {
    ...
    let result: (f64, Vec<f64>) = fermi_occupation(orbe.view(), n_elec, n_elec_unpaired, temperature);
    let mu: f64 = result.0;
