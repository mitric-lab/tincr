...
use crate::fmo::{Pair, Monomer, ESDPair};
use crate::scc::h0_and_s::*;
use crate::scc::gamma_approximation::*;
use crate::scc::{density_matrix_ref, lc_exact_exchange, density_matrix, get_repulsive_energy, get_electronic_energy, calc_exchange, get_electronic_energy_new};
...
impl Pair {
    ...
    pub fn run_scc_test(&mut self, atoms: &[Atom], config: SccConfig) {
        let scf_charge_conv: f64 = config.scf_charge_conv;
        let scf_energy_conv: f64 = config.scf_energy_conv;
        let max_iter: usize = config.scf_max_cycles;
        let mut p: Array2<f64> = self.properties.take_p().unwrap();
