...
impl SuperSystem {
    ...
    pub fn le_le_coupling_grad_new(&mut self, i: &ReducedLE, j: &ReducedLE) -> Array1<f64> {
        ...
        let (state_i,state_j,homo_i,homo_j):(usize,usize,usize,usize) = if i.monomer_index < j.monomer_index{
        ...
        if type_pair == PairType::Pair {
            ...
            for nat in 0..3*m_i.n_atoms{
                cphf_tdm_i.slice_mut(s![nat,..,..]).assign(&((dc_mo_i.slice(s![nat,..nocc_i,..]).t()
                    ...
                    (c_mo_i.slice(s![..,..nocc_i])
                        .dot(&cis_c_i).dot(&dc_mo_i.slice(s![nat,nocc_i..,..]))))
                );
            }
            for nat in 0..3*m_j.n_atoms{
                cphf_tdm_j.slice_mut(s![3*m_i.n_atoms+nat,..,..]).assign(&((dc_mo_j.slice(s![3*m_i.n_atoms+nat,..nocc_j,..]).t()
                    ...
                    (c_mo_j.slice(s![..,..nocc_j])
                        .dot(&cis_c_j).dot(&dc_mo_j.slice(s![3*m_i.n_atoms+nat,nocc_j..,..]))))
                );
            }
            ...
            // calculate gradients of the MO coefficients for ALL occupied and virtual orbitals
            // dc_mu,i/dR = sum_m^all U^R_mi, C_mu,m
            let mut dc_mo_i:Array3<f64> = Array3::zeros([3*esd_pair.n_atoms,m_i.n_orbs,m_i.n_orbs]);
            let mut dc_mo_j:Array3<f64> = Array3::zeros([3*esd_pair.n_atoms,m_i.n_orbs,m_j.n_orbs]);
            ...
            let mut cphf_tdm_i:Array3<f64> = Array3::zeros([3*esd_pair.n_atoms,m_i.n_orbs,m_i.n_orbs]);
            let mut cphf_tdm_j:Array3<f64> = Array3::zeros([3*esd_pair.n_atoms,m_j.n_orbs,m_j.n_orbs]);
            for nat in 0..3*m_i.n_atoms{
                cphf_tdm_i.slice_mut(s![nat,..,..]).assign(&((dc_mo_i.slice(s![nat,..nocc_i,..]).t()
                    .dot(&cis_c_i).dot(&c_mo_i.slice(s![..,nocc_i..]).t())) +
                    (c_mo_i.slice(s![..,..nocc_i])
            ...
            // transform cphf tdm matrices to the shape [grad,norb*norb]
            let cphf_tdm_i:Array2<f64> = cphf_tdm_i.into_shape([3*esd_pair.n_atoms,m_i.n_orbs*m_i.n_orbs]).unwrap();
            let cphf_tdm_j:Array2<f64> = cphf_tdm_j.into_shape([3*esd_pair.n_atoms,m_j.n_orbs*m_j.n_orbs]).unwrap();
            let tdm_i:Array1<f64> = tdm_i.into_shape([m_i.n_orbs*m_i.n_orbs]).unwrap();
            let tdm_j:Array1<f64> = tdm_j.into_shape([m_j.n_orbs*m_j.n_orbs]).unwrap();
        ...
        if i.monomer_index == j.electron.m_index {
            ...
            if type_ij == PairType::Pair {
                ...
                let grad = if pair.i == i.monomer_index {
                    ...
                    for nat in 0..3*m_j.n_atoms{
                        dc_mo_j.slice_mut(s![3*m_i.n_atoms+nat,..]).assign(&u_mat_j.slice(s![nat,..,j.hole.mo.index]).dot(&c_mo_j.t()));
                    }
                    let mut cphf_tdm_i:Array3<f64> = Array3::zeros([3*pair.n_atoms,m_i.n_orbs,m_i.n_orbs]);
                    for nat in 0..3*m_i.n_atoms{
                        cphf_tdm_i.slice_mut(s![nat,..,..]).assign(&((dc_mo_i.slice(s![nat,..nocc,..]).t()
                            .dot(&cis_c).dot(&c_mo_i.slice(s![..,nocc..]).t())) +
                            (c_mo_i.slice(s![..,..nocc])
                                .dot(&cis_c).dot(&dc_mo_i.slice(s![nat,nocc..,..]))))
                        );
                    }
                    // transform cphf tdm matrices to the shape [grad,norb*norb]
                    let cphf_tdm_i:Array2<f64> = cphf_tdm_i.into_shape([3*pair.n_atoms,m_i.n_orbs*m_i.n_orbs]).unwrap();
                    ...
                    let coulomb_integral:Array2<f64> = coulomb_integral
                        .into_shape([n_orbs_i*n_orbs_i,n_orbs_i*n_orbs_j]).unwrap();

                    let c_i_ind:ArrayView1<f64> = c_mo_i.slice(s![..,j.electron.mo.index]);
                    let c_j_ind:ArrayView1<f64> = c_mo_j.slice(s![..,j.hole.mo.index]);
                    let derivative_c_i:ArrayView2<f64> = dc_mo_i.slice(s![..,j.electron.mo.index,..]);

                    // calculate (d/dr tdm_i * C^J_hole * C^I_elec + tdm_i * d/dr C^J_hole * C^I_elec + tdm_i * C^J_hole * d/dr C^I_elec)
                    let term_1:Array1<f64> = cphf_tdm_i.dot(&coulomb_integral.dot(&(into_col(c_i_ind.to_owned())
                        .dot(&into_row(c_j_ind.to_owned()))).into_shape([n_orbs_i*n_orbs_j]).unwrap()));
                    let term_2:Array1<f64> = derivative_c_i.dot(&tdm_i.dot(&coulomb_integral)
                        .into_shape([n_orbs_i,n_orbs_j]).unwrap().dot(&c_j_ind));
                    let term_3:Array1<f64> = c_i_ind.dot(&tdm_i.dot(&coulomb_integral)
                        .into_shape([n_orbs_i,n_orbs_j]).unwrap()).dot(&dc_mo_j.t());
                    ...
                    for nat in 0..3*m_j.n_atoms{
                        for orb in 0..m_j.n_orbs{
                            dc_mo_j.slice_mut(s![3*m_i.n_atoms+nat,orb,..])
                                .assign(&u_mat_j.slice(s![nat,..,orb]).dot(&c_mo_j.t()));
                        }
                    }
                    for nat in 0..3*m_i.n_atoms{
                        dc_mo_i.slice_mut(s![nat,..])
                            .assign(&u_mat_j.slice(s![nat,..,j.hole.mo.index]).dot(&c_mo_i.t()));
                    }
                    let mut cphf_tdm_j:Array3<f64> = Array3::zeros([3*pair.n_atoms,m_j.n_orbs,m_j.n_orbs]);
                    for nat in 0..3*m_j.n_atoms{
                        cphf_tdm_j.slice_mut(s![nat,..,..]).assign(&((dc_mo_j.slice(s![nat,..nocc,..]).t()
                            .dot(&cis_c).dot(&c_mo_j.slice(s![..,nocc..]).t())) +
                            (c_mo_j.slice(s![..,..nocc])
                                .dot(&cis_c).dot(&dc_mo_j.slice(s![nat,nocc..,..]))))
                        );
                    }
                    // transform cphf tdm matrices to the shape [grad,norb*norb]
                    let cphf_tdm_j:Array2<f64> = cphf_tdm_j.into_shape([3*pair.n_atoms,m_j.n_orbs*m_j.n_orbs]).unwrap();
                    ...
                    coulomb_integral = 2.0 * coulomb_integral - exchange_integral;
                    let coulomb_integral:Array2<f64> = coulomb_integral
                        .into_shape([n_orbs_j*n_orbs_j,n_orbs_j*n_orbs_i]).unwrap();

                    let c_j_ind:ArrayView1<f64> = c_mo_i.slice(s![..,j.electron.mo.index]);
                    let c_i_ind:ArrayView1<f64> = c_mo_j.slice(s![..,j.hole.mo.index]);
                    let derivative_c_j:ArrayView2<f64> = dc_mo_j.slice(s![..,j.electron.mo.index,..]);

                    // calculate (d/dr tdm_i * C^J_hole * C^I_elec + tdm_i * d/dr C^J_hole * C^I_elec + tdm_i * C^J_hole * d/dr C^I_elec)
                    let term_1:Array1<f64> = cphf_tdm_j.dot(&coulomb_integral.dot(&(into_col(c_j_ind.to_owned())
                        .dot(&into_row(c_i_ind.to_owned()))).into_shape([n_orbs_j*n_orbs_i]).unwrap()));
                    let term_2:Array1<f64> = derivative_c_j.dot(&tdm_j.dot(&coulomb_integral)
                        .into_shape([n_orbs_j,n_orbs_i]).unwrap().dot(&c_i_ind));
                    let term_3:Array1<f64> = c_j_ind.dot(&tdm_j.dot(&coulomb_integral)
                        .into_shape([n_orbs_j,n_orbs_i]).unwrap()).dot(&dc_mo_i.t());
            ...
            if type_ik == PairType::Pair {
                ...
                let grad = if pair.i == i.monomer_index {
                    ...
                    for nat in 0..3*m_j.n_atoms{
                        dc_mo_j.slice_mut(s![3*m_i.n_atoms+nat,..])
                            .assign(&u_mat_j.slice(s![nat,..,j.electron.mo.index]).dot(&c_mo_j.t()));
                    }
                    let mut cphf_tdm_i:Array3<f64> = Array3::zeros([3*pair.n_atoms,m_i.n_orbs,m_i.n_orbs]);
                    for nat in 0..3*m_i.n_atoms{
                        cphf_tdm_i.slice_mut(s![nat,..,..]).assign(&((dc_mo_i.slice(s![nat,..nocc,..]).t()
                    ...
                    let derivative_c_i:ArrayView2<f64> = dc_mo_i.slice(s![..,j.hole.mo.index,..]);
                    ...
                    for nat in 0..3*m_i.n_atoms{
                        dc_mo_i.slice_mut(s![nat,..])
                            .assign(&u_mat_j.slice(s![nat,..,j.electron.mo.index]).dot(&c_mo_i.t()));
                    }
                    let mut cphf_tdm_j:Array3<f64> = Array3::zeros([3*pair.n_atoms,m_j.n_orbs,m_j.n_orbs]);
                    for nat in 0..3*m_j.n_atoms{
                        cphf_tdm_j.slice_mut(s![nat,..,..]).assign(&((dc_mo_j.slice(s![nat,..nocc,..]).t()
                    ...
                    let derivative_c_j:ArrayView2<f64> = dc_mo_j.slice(s![..,j.hole.mo.index,..]);
            ...
            CTCoupling::IJIJ => {
                if type_ij == PairType::Pair {
                    ...
                    let grad = if pair.i == i.m_index {
                        ...
                        // dc_mu,i/dR = sum_m^all U^R_mi, C_mu,m
                        let mut dc_mo_i:Array2<f64> = Array2::zeros([3*pair.n_atoms,m_i.n_orbs]);
                        let mut dc_mo_k:Array2<f64> = Array2::zeros([3*pair.n_atoms,m_i.n_orbs]);
                        let mut dc_mo_j:Array2<f64> = Array2::zeros([3*pair.n_atoms,m_j.n_orbs]);
                        let mut dc_mo_l:Array2<f64> = Array2::zeros([3*pair.n_atoms,m_j.n_orbs]);
                        // iterate over gradient dimensions of both monomers
                        for nat in 0..3*m_i.n_atoms{
                            dc_mo_i.slice_mut(s![nat,..]).assign(&umat_i.slice(s![nat,..,i.mo.index]).dot(&c_mo_i.t()));
                            dc_mo_k.slice_mut(s![nat,..]).assign(&umat_i.slice(s![nat,..,k.mo.index]).dot(&c_mo_i.t()));
                        }
                        for nat in 0..3*m_j.n_atoms{
                            dc_mo_j.slice_mut(s![3*m_i.n_atoms+nat,..]).assign(&umat_j.slice(s![nat,..,j.mo.index]).dot(&c_mo_j.t()));
                            dc_mo_l.slice_mut(s![3*m_i.n_atoms+nat,..]).assign(&umat_j.slice(s![nat,..,l.mo.index]).dot(&c_mo_j.t()));
                        }
                        ...
                        let coulomb_arr:Array2<f64> =  coulomb_arr.into_shape([m_i.n_orbs*m_i.n_orbs,m_j.n_orbs*m_j.n_orbs]).unwrap();
                        let c_mat_jl:Array2<f64> = into_col(c_j_ind.to_owned())
                            .dot(&into_row(c_l_ind.to_owned()));
                        let c_mat_ik:Array2<f64> = into_col(c_i_ind.to_owned())
                        ...
                        for nat in 0..3*pair.n_atoms{
                            ...
                            let term_1a = c_i.into_shape(m_i.n_orbs*m_i.n_orbs).unwrap()
                                .dot(&coulomb_arr.dot(&c_mat_jl.view().into_shape(m_j.n_orbs*m_j.n_orbs).unwrap()));
                            let term_1b = c_i_2.into_shape(m_i.n_orbs*m_i.n_orbs).unwrap()
                                .dot(&coulomb_arr.dot(&c_mat_jl.view().into_shape(m_j.n_orbs*m_j.n_orbs).unwrap()));
                            let term_2a = c_mat_ik.view().into_shape(m_i.n_orbs*m_i.n_orbs).unwrap()
                                .dot(&coulomb_arr.dot(&c_j.into_shape(m_j.n_orbs*m_j.n_orbs).unwrap()));
                            let term_2b = c_mat_ik.view().into_shape(m_i.n_orbs*m_i.n_orbs).unwrap()
                        ...
                        // calculate gradients of the MO coefficients
                        // dc_mu,i/dR = sum_m^all U^R_mi, C_mu,m
                        let mut dc_mo_i:Array2<f64> = Array2::zeros([3*pair.n_atoms,m_j.n_orbs]);
                        let mut dc_mo_k:Array2<f64> = Array2::zeros([3*pair.n_atoms,m_j.n_orbs]);
                        let mut dc_mo_j:Array2<f64> = Array2::zeros([3*pair.n_atoms,m_i.n_orbs]);
                        let mut dc_mo_l:Array2<f64> = Array2::zeros([3*pair.n_atoms,m_i.n_orbs]);
                        // iterate over gradient dimensions of both monomers
                        for nat in 0..3*m_j.n_atoms{
                            dc_mo_i.slice_mut(s![3*m_i.n_atoms+nat,..]).assign(&umat_i.slice(s![nat,..,i.mo.index]).dot(&c_mo_j.t()));
                            dc_mo_k.slice_mut(s![3*m_i.n_atoms+nat,..]).assign(&umat_i.slice(s![nat,..,k.mo.index]).dot(&c_mo_j.t()));
                        }
                        for nat in 0..3*m_i.n_atoms{
                            dc_mo_j.slice_mut(s![nat,..]).assign(&umat_j.slice(s![nat,..,j.mo.index]).dot(&c_mo_i.t()));
                            dc_mo_l.slice_mut(s![nat,..]).assign(&umat_j.slice(s![nat,..,l.mo.index]).dot(&c_mo_i.t()));
                        }
                        ...
                        for nat in 0..3*pair.n_atoms{
                            ...
                            // calculate dot product of coulomb integral with previously calculated coefficients
                            // in AO basis
                            let term_1a = c_mat_jl.view().into_shape(m_i.n_orbs*m_i.n_orbs).unwrap()
                                .dot(&coulomb_arr.dot(&c_i.view().into_shape(m_j.n_orbs*m_j.n_orbs).unwrap()));
                            let term_1b = c_mat_jl.view().into_shape(m_i.n_orbs*m_i.n_orbs).unwrap()
                                .dot(&coulomb_arr.dot(&c_i_2.view().into_shape(m_i.n_orbs*m_i.n_orbs).unwrap()));
                            let term_2a = c_j.view().into_shape(m_i.n_orbs*m_i.n_orbs).unwrap()
                                .dot(&coulomb_arr.dot(&c_mat_ik.view().into_shape(m_j.n_orbs*m_j.n_orbs).unwrap()));
                            let term_2b = c_j_2.view().into_shape(m_i.n_orbs*m_i.n_orbs).unwrap()
                                .dot(&coulomb_arr.dot(&c_mat_ik.view().into_shape(m_j.n_orbs*m_j.n_orbs).unwrap()));
            ...
            CTCoupling::IJJI => {
                ...
                if type_ij == PairType::Pair {
                    ...
                    let grad = if m_j.index == j.m_index {
                        ...
                        for nat in 0..3*m_i.n_atoms{
                            dc_mo_i.slice_mut(s![nat,..]).assign(&umat_i.slice(s![nat,..,i.mo.index]).dot(&c_mo_i.t()));
                            dc_mo_l.slice_mut(s![nat,..]).assign(&umat_i.slice(s![nat,..,l.mo.index]).dot(&c_mo_i.t()));
                        }
                        for nat in 0..3*m_j.n_atoms{
                            dc_mo_j.slice_mut(s![3*m_i.n_atoms+nat,..]).assign(&umat_j.slice(s![nat,..,j.mo.index]).dot(&c_mo_j.t()));
                            dc_mo_k.slice_mut(s![3*m_i.n_atoms+nat,..]).assign(&umat_j.slice(s![nat,..,k.mo.index]).dot(&c_mo_j.t()));
                        }
                        ...
                        for nat in 0..3*m_j.n_atoms{
                            dc_mo_i.slice_mut(s![3*m_i.n_atoms+nat,..]).assign(&umat_i.slice(s![nat,..,i.mo.index]).dot(&c_mo_j.t()));
                            dc_mo_l.slice_mut(s![3*m_i.n_atoms+nat,..]).assign(&umat_i.slice(s![nat,..,l.mo.index]).dot(&c_mo_j.t()));
                        }
                        for nat in 0..3*m_i.n_atoms{
                            dc_mo_j.slice_mut(s![nat,..]).assign(&umat_j.slice(s![nat,..,j.mo.index]).dot(&c_mo_i.t()));
                            dc_mo_k.slice_mut(s![nat,..]).assign(&umat_j.slice(s![nat,..,k.mo.index]).dot(&c_mo_i.t()));
                        }
