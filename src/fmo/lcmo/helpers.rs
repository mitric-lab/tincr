...
pub fn le_le_two_electron_loop(
    ...
    for state_i in 0..n_le {
        for state_j in 0..n_le {
            for mu in 0..n_orb_i {
                for nu in 0..n_orb_i {
                    let t_mu_nu: f64 = orbs_i.slice(s![mu, ..occ_index_end_i]).dot(
                        &exc_coeff_i
                            .slice(s![state_i, .., ..])
                    ...
                    for lambda in 0..n_orb_j {
                        for sigma in 0..n_orb_j {
                            let t_lambda_sigma: f64 =
                                orbs_i.slice(s![lambda, ..occ_index_end_j]).dot(
                                    &exc_coeff_j
                                        .slice(s![state_j, .., ..])
                                        .dot(&orbs_i.slice(s![sigma, virt_index_start_j..])),
                                );

                            coupling_matrix[[state_i, state_j]] += 0.25
                                * t_mu_nu
                                * t_lambda_sigma
                                * (2.0
                                    * s_pair[[mu, nu]]
                                    * s_pair[[(n_orb_i - 1) + lambda, (n_orb_i - 1) + sigma]]
                                    * (g0_ao_pair[[mu, (n_orb_i - 1) + lambda]]
                                        + g0_ao_pair[[mu, (n_orb_i - 1) + sigma]]
                                        + g0_ao_pair[[nu, (n_orb_i - 1) + lambda]]
                                        + g0_ao_pair[[nu, (n_orb_i - 1) + sigma]])
                                    - s_pair[[mu, (n_orb_i - 1) + lambda]]
                                        * s_pair[[nu, (n_orb_i - 1) + sigma]]
                                        * (g0_ao_pair[[mu, nu]]
                                            + g0_ao_pair[[mu, (n_orb_i - 1) + sigma]]
                                            + g0_ao_pair[[(n_orb_i - 1) + lambda, nu]]
                                            + g0_ao_pair
                                                [[(n_orb_i - 1) + lambda, (n_orb_i - 1) + sigma]]));
                        }
                    }
                }
            }
                ...
                for lambda in 0..n_orb_i {
                    for nu in 0..n_orb_j {
                        for sigma in 0..n_orb_j {
                            ...
                            exchange[[occ, virt]] += 0.25
                                * orbs_i[[mu, occ_i]]
                                * orbs_i[[lambda, occ_i]]
                                * orbs_j[[nu, virt_j]]
                                * orbs_j[[sigma, virt_j]]
                                * s_ij[[mu, nu_pair]]
                                * s_ij[[lambda, sigma_pair]]
                                * (g0_pair[[mu, lambda]]
                                    + g0_pair[[mu, sigma_pair]]
                                    + g0_pair[[nu_pair, lambda]]
                                    + g0_pair[[nu_pair, sigma_pair]]);
                        }
                    }
                }
            }
