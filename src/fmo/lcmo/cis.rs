...
impl SuperSystem{
    pub fn build_cis_matrix(&self,lcmo_h:ArrayView2<f64>){
        ...
        for pair in self.pairs.iter(){
            ...
            let cis_index_ct:usize = dim_le + index_i * (self.n_mol-1) *n_ct + (index_j-1) *n_ct;
            cis_matrix.slice_mut(s![cis_index_le..cis_index_le+n_le,cis_index_ct..cis_index_ct+n_ct]).add_assign(&le_i_ct_ij);
            ...
            cis_matrix.slice_mut(s![cis_index_le..cis_index_le+n_le,cis_index_ct..cis_index_ct+n_ct]).add_assign(&le_i_ct_ji);
            ...
            for i in 0..n_ct{
                cis_matrix[[cis_index_ab+i,cis_index_ab+i]] += lcmo_h[[index_i,index_i]] - lcmo_h[[index_j,index_j]];
                cis_matrix[[cis_index_ba+i,cis_index_ba+i]] += lcmo_h[[index_j,index_j]] - lcmo_h[[index_i,index_i]];
            }
            cis_matrix.slice_mut(s![cis_index_ab..(cis_index_ab+n_ct),cis_index_ab..(cis_index_ab+n_ct)]).add_assign(&(exchange_ij-coulomb_ij));
            cis_matrix.slice_mut(s![cis_index_ba..(cis_index_ba+n_ct),cis_index_ba..(cis_index_ba+n_ct)]).add_assign(&(exchange_ji-coulomb_ji));
            ...
            let (qoo_lele_ij,qvv_lele_ij):(Array2<f64>,Array2<f64>) = inter_fragment_trans_charges_oovv(
                &atoms[self.monomers[index_i].slice.atom_as_range()],
                &atoms[self.monomers[index_j].slice.atom_as_range()],
                c_mo_i,
                c_mo_j,
            ...
            let (qoo_lect_ii,qvv_lect_ii):(Array2<f64>,Array2<f64>) = inter_fragment_trans_charges_oovv(
                &atoms[self.monomers[index_i].slice.atom_as_range()],
                &atoms[self.monomers[index_i].slice.atom_as_range()],
                c_mo_i,
                c_mo_i,
            ...
            // interfragment transition charges
            let (qoo_lect_ij,qvv_lect_ij):(Array2<f64>,Array2<f64>) = inter_fragment_trans_charges_oovv(
                &atoms[self.monomers[index_i].slice.atom_as_range()],
                &atoms[self.monomers[index_j].slice.atom_as_range()],
                c_mo_i,
                c_mo_j,
            ...
            cis_matrix.slice_mut(s![(index_i * n_le)..(index_i * n_le)+n_le,cis_index_ab..(cis_index_ab+n_ct)]).assign(&le_ct_matrix_ji);
            cis_matrix.slice_mut(s![(index_i * n_le)..(index_i * n_le)+n_le,cis_index_ba..(cis_index_ba+n_ct)]).assign(&le_ct_matrix_ij);
            ...
            let (qoo_ctct_ij,qvv_ctct_ij):(Array2<f64>,Array2<f64>) = inter_fragment_trans_charges_oovv(
                &atoms[self.monomers[index_i].slice.atom_as_range()],
                &atoms[self.monomers[index_j].slice.atom_as_range()],
                c_mo_i,
                c_mo_j,
            ...
            let qoo_ctct_ji:Array2<f64> =
                qoo_ctct_ij.into_shape([n_atoms_i+n_atoms_j,n_occ_m,n_occ_m]).unwrap()
                    .permuted_axes([0,2,1]).as_standard_layout().
                    into_shape([n_atoms_i+n_atoms_j,n_occ_m*n_occ_m]).unwrap().to_owned();
