...
use crate::fmo::lcmo::helpers::{coulomb_integral_loop_ao, exchange_integral_loop_ao};
use crate::fmo::{
...
impl SuperSystem {
    ...
    pub fn calculate_cphf_correction(
        &mut self,
        index_i: usize,
        index_j: usize,
        ct_ind_i: usize,
        ...
        println!(
            "Elapsed time Calculation of both U matrices {:>8.6}",
        ...
        let cphf_gradient: Array1<f64> = if hole_i {
            ...
            for nat in 0..3 * m_i.n_atoms {
                dc_mo_i
                    .slice_mut(s![nat, ..])
                    .assign(&u_mat_i.slice(s![nat, .., orb_ind_i]).dot(&c_mo_i.t()));
            }
            for nat in 0..3 * m_j.n_atoms {
                dc_mo_j
                    .slice_mut(s![3 * m_i.n_atoms + nat, ..])
                    .assign(&u_mat_j.slice(s![nat, .., orb_ind_j]).dot(&c_mo_j.t()));
            }
            ...
            for nat in 0..3 * pair_ij.n_atoms {
                ...
                let term_1a = c_i.into_shape(m_i.n_orbs * m_i.n_orbs).unwrap().dot(
                    &coulomb_arr.dot(&c_mat_j.view().into_shape(m_j.n_orbs * m_j.n_orbs).unwrap()),
                );
            ...
            println!(
                "Elapsed time other version {:>8.6}",
                timer.elapsed().as_secs_f64()
            );
            drop(timer);
            // assert!(coulomb_grad_2.abs_diff_eq(&coulomb_grad,1.0e-11));
