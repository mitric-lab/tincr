...
impl SuperSystem {
    pub fn cpcis_routine(
        ...
        let bmat_2d: Array2<f64> = bmat
            .clone()
            .permuted_axes([0, 2, 1])
            .as_standard_layout()
            .to_owned()
            .into_shape([3 * m_i.n_atoms, nocc * nvirt])
            .unwrap();
        // solve the equation
...
impl Monomer {
    pub fn calculate_cpcis_terms(
        ...
        let integrals_2d: Array2<f64> = integrals
            .into_shape([self.n_orbs * self.n_orbs, self.n_orbs * self.n_orbs])
            .unwrap();
        ...
        let cis_coeff: Array1<f64> = self.properties.ci_coefficient(le_state).unwrap().to_owned();
        let cis_coeff: Array2<f64> = cis_coeff.into_shape([nocc, nvirt]).unwrap();
        let cis_coeff:Array2<f64> = cis_coeff.permuted_axes([1,0]).as_standard_layout().to_owned();
        // CIS coefficients in AO basis
        let cis_coeff_ao: Array2<f64> = orbs_virt.dot(&cis_coeff.dot(&orbs_occ.t()));
        ...
        let integral_dot_cis_ao: Array2<f64> = integrals_2d
            .dot(
                &cis_coeff_ao
                    .view()
        ...
        for nc in 0..3 * self.n_atoms {
            ...
            // let integral_derivs_2d: ArrayView2<f64> = integral_derivatives
            //     .slice(s![nc, .., .., .., ..])
            //     .into_shape([self.n_orbs * self.n_orbs, self.n_orbs * self.n_orbs])
            //     .unwrap();
            // let integrals_test_2:Array2<f64> = integral_derivs_2d.dot(&cis_coeff_ao.view()
            //     .into_shape([self.n_orbs*self.n_orbs]).unwrap())
            //     .into_shape([self.n_orbs,self.n_orbs]).unwrap();
            // let integrals_test:ArrayView2<f64> = deriv_integral_cis_ao.slice(s![nc,..,..]);
            // assert!(integrals_test.abs_diff_eq(&integrals_test_2,1e-9),"Integral derivatives not equal!");

            // b_w * w
            let term_1: Array2<f64> = cis_energy * &cis_coeff;

            // (C^x_v F C_v + C_v F^X C_V + C_v F C^x_v) b_w
            let term_2: Array2<f64> = (dc_mo_v.t().dot(&fock_mat.dot(&orbs_virt))
                + orbs_virt
                    .t()
                    .dot(&fock_derivative.slice(s![nc, .., ..]).dot(&orbs_virt))
                + orbs_virt.t().dot(&fock_mat.dot(&dc_mo_v)))
            .dot(&cis_coeff);

            // b_w(C^x_o F C_o + C_o F^X C_o + C_o F C^x_o)
            let term_3: Array2<f64> = cis_coeff.dot(
                &(dc_mo_o.t().dot(&fock_mat.dot(&orbs_occ))
                    + orbs_occ
                        .t()
                        .dot(&fock_derivative.slice(s![nc, .., ..]).dot(&orbs_occ))
            ...
            l_b.slice_mut(s![nc, .., ..])
                .assign(&(term_1 - term_2 - term_3 + term_4 + term_5));
        }
        ...
        //     .as_standard_layout()
        //     .to_owned();
        // let integrals_mo_2: Array2<f64> = integrals_mo_2
        //     .into_shape([nvirt * nocc, nvirt * nocc])
        //     .unwrap();
    ...
    pub fn fock_derivative(&mut self, atoms: &[Atom], u_mat: ArrayView3<f64>) -> Array3<f64> {
        ...
        for nc in 0..3 * self.n_atoms {
            ...
            b_mat.slice_mut(s![nc, .., ..]).assign(
                &(&gradh_mo
                    - &u_mat.slice(s![nc, .., ..]) * &orbe_diff_matrix
                    - &grads_mo * &orbe_matrix
                    + &a_dot_u),
            );
            b_mat
            ...
            for nu in 0..n_orbs {
                for sig in 0..n_orbs {
                    ...
                    exchange_integral[[mu, sig, nu, la]] += 0.25
                        * s[[mu, sig]]
                        * s[[nu, la]]
                        * (g0_lr[[mu, nu]] + g0_lr[[mu, la]] + g0_lr[[sig, nu]] + g0_lr[[sig, la]]);
                }
            }
        }
    }
    let exchange_integral: Array4<f64> = exchange_integral
        .permuted_axes([0, 3, 2, 1])
        .as_standard_layout()
        .to_owned();
    return 2.0 * coulomb_integral - exchange_integral;
...
fn f_monomer_coulomb_exchange_loop_new(
    ...
    let exchange_integral: Array5<f64> = exchange_integral
        .permuted_axes([0, 1, 4, 3, 2])
        .as_standard_layout()
        .to_owned();
    return 2.0 * coulomb_integral - exchange_integral;
...
fn solve_cpcis_pople_test(
    ...
    for nc in 0..3 * n_atoms {
        let b_zero: ArrayView1<f64> = matrix
            .slice(s![nc, .., ..])
            .into_shape([nvirt * nocc])
            .unwrap();
        ...
        'cphf_loop: for it in 0..40 {
            ...
            if it >= 2 {
                ...
                let not_converged: Vec<f64> = diff
                    .iter()
                    .filter_map(|&item| if item > 1e-7 { Some(item) } else { None })
                    .collect();
                u_prev = u_mat_1d;
        ...
        cis_der
            .slice_mut(s![nc, .., ..])
            .assign(&u_prev.into_shape([nvirt, nocc]).unwrap());
    }
...
fn solve_cpcis_inversion(
    ...
    let integrals_mo: Array4<f64> = integrals_2d
        ...
        .as_standard_layout()
        .to_owned();
    let amat: Array2<f64> = integrals_mo
        .into_shape([nvirt * nocc, nvirt * nocc])
        .unwrap()
        / energy;
