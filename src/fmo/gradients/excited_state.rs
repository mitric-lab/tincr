use crate::fmo::{Monomer, Pair, SuperSystem, ExcitedStateMonomerGradient, ESDPair};
...
impl Pair{
    pub fn prepare_ct_state_parallel(
        &self,
        m_i:&Monomer,
        m_j:&Monomer,
        ct_ind_i: usize,
        ...
        let occ_indices = self.properties.occ_indices().unwrap().to_vec();
        let virt_indices = self.properties.virt_indices().unwrap().to_vec();
        ...
        let ci_coefficients:Array2<f64> = if hole_i == true{
            ...
            // save in properties
           cis_coeff.into_shape([1,nocc*nvirt]).unwrap()
        }
    ...
    pub fn tda_gradient_lc_parallel_ct(&self, state: usize,cis_coefficients:ArrayView2<f64>,cis_energy:f64)->Array1<f64>{
        // get occ and virt indices from properties
        let occ_indices:&[usize] = self.properties.occ_indices().unwrap();
        let virt_indices:&[usize] = self.properties.virt_indices().unwrap();
        ...
        let omega_state:f64 = cis_energy;
        // take state specific values from the excitation vectors
        let x_state:ArrayView3<f64> = cis_coefficients
...
impl ESDPair{
    ...
    pub fn tda_gradient_nolc(&self, state: usize)->Array1<f64>{
        // get occ and virt indices from properties
        let occ_indices:&[usize] = self.properties.occ_indices().unwrap();
        let virt_indices:&[usize] = self.properties.virt_indices().unwrap();
    ...
    pub fn tda_gradient_nolc_parllel_ct(&self, state: usize,cis_coefficients:ArrayView2<f64>,ct_energy:f64)->Array1<f64>{
        // get occ and virt indices from properties
        let occ_indices:&[usize] = self.properties.occ_indices().unwrap();
        let virt_indices:&[usize] = self.properties.virt_indices().unwrap();
