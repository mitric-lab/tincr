use core::::ProductCache;
use tincr::src::monomer::Monomer;
use core::::System;
use tincr::scc::scc_routine::RestrictedSCC;
use ndarray::prelude::*;
use ndarray::Data;
pub use utils::*;
...
impl<'a> DavidsonEngine for Monomer<'a> {
    /// The products of the TDA/CIS-Hamiltonian with the subspace vectors is computed.
    fn compute_products<'b>(&mut self, x: ArrayView2<'b, f64>) -> Array2<f64> {
        ...
        if self.gammafunction_lc.is_some() {
            ...
            let gamma_oo: Array2<f64> = gamma_lr
                .dot(&q_oo)
                .into_shape([self.atoms.len() * n_occ, n_occ])
                .unwrap();
                ...
                .enumerate()
            {
                ...
                let q_vv_r = q_vv.into_shape((self.atoms.len() * n_virt, n_virt)).unwrap();
                ...
                k.assign(
                    ...
                    &gamma_oo.t().dot(
                        &xi.dot(&q_vv_r.t()) //xi: nocc, nvirt | qvvrT: nvirt, natoms*nvirt
                            .into_shape((n_occ, self.atoms.len(), n_virt))
                            .unwrap()
                            .permuted_axes([1, 0, 2]) // natoms, nocc, nvirt
                            .as_standard_layout()
                            .into_shape((self.atoms.len() * n_occ, n_virt))
                            .unwrap(),
...
impl<'a> DavidsonEngine for System<'a> {
    /// The products of the TDA/CIS-Hamiltonian with the subspace vectors is computed.
    fn compute_products<'b>(&mut self, x: ArrayView2<'b, f64>) -> Array2<f64> {
        // Mutable reference to the product cache.
        let mut cache: ProductCache = self.data.take_cache();
