...
impl<'a> Monomer<'a> {
    pub fn compute_q_ov(&mut self, atoms: &[Atom], s_c: Option<ArrayView2<f64>>) -> Array2<f64>{
        ...
        let mut q_trans: Array3<f64> = Array3::zeros([self.atoms.len(), n_occ, n_virt]);
        ...
        q_trans.into_shape([self.atoms.len(), n_occ * n_virt]).unwrap()
    }
    ...
    pub fn compute_q_vo(&mut self, atoms: &[Atom], s_c: Option<ArrayView2<f64>>) -> Array2<f64>{
        ...
        let mut q_trans: Array3<f64> = Array3::zeros([self.atoms.len(), n_virt, n_occ]);
