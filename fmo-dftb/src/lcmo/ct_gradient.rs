...
impl<'a> SuperSystem<'a> {
    pub fn ct_gradient(
        ...
        let (grad_s_pair, grad_h0_pair) = &pair_ij.slako.h0_and_s_gradients(&pair_atoms, pair_ij.n_orbs());
        ...
        if !pair_ij.data.s_is_set() {
            ...
            let (s_ab, h0_ab): (Array2<f64>, Array2<f64>) = m_i.slako.h0_and_s_ab(
                m_i.n_orbs(),
                m_j.n_orbs(),
                &pair_atoms[0..m_i.atoms.len()],
                &pair_atoms[m_i.atoms.len()..]
            );
        ...
        if !pair_ij.data.gamma_is_set() {
            let a: usize = m_i.atoms.len();
            let mut gamma_pair: Array2<f64> = Array2::zeros([pair_ij.atoms.len(), pair_ij.atoms.len()]);
            let gamma_ab: Array2<f64> = gamma_atomwise_ab(
                &pair_ij.gammafunction,
                &pair_atoms[0..m_i.atoms.len()],
                &pair_atoms[m_j.atoms.len()..],
                m_i.atoms.len(),
                m_j.atoms.len(),
            );
            gamma_pair
        ...
        .into_shape([3 * pair_ij.atoms.len(), n_orbs_i * n_orbs_i])
        .unwrap()
...
impl<'a> Monomer<'a> {
    pub fn calculate_ct_fock_gradient(
        ...
        let (grad_s, grad_h0) = self.slako.h0_and_s_gradients(&atoms, self.n_orbs());
        ...
        let mut gradient_term:Array1<f64> = Array1::zeros(3*self.atoms.len());
        // calculate transition charges
        let (qov,qoo,qvv) = trans_charges(self.atoms.len(),atoms,orbs,s,occ_indices,virt_indices);
        // virtual-occupied transition charges
        let qvo:Array2<f64> = qov.clone().into_shape([self.atoms.len(),nocc,nvirt]).unwrap()
            .permuted_axes([0, 2, 1])
            .as_standard_layout()
            .to_owned().into_shape([self.atoms.len(),nvirt*nocc]).unwrap();
        ...
        if hole {
            ...
            // in equation (25)
            // integral (ii|kl)
            let mut integral:Array2<f64> = 2.0* qoo.view().into_shape([self.atoms.len(),nocc,nocc]).unwrap().slice(s![..,ind,ind]).dot(&g0.dot(&qoo))
                .into_shape([nocc,nocc]).unwrap();

            if self.gammafunction_lc.is_some(){
                let g0_lr:ArrayView2<f64> = self.data.gamma_lr();
                // integral (ik|il)
                let term_2:Array2<f64> = qoo.view().into_shape([self.atoms.len(),nocc,nocc]).unwrap().slice(s![..,ind,..]).t()
                    .dot(&g0_lr.dot(&qoo.view().into_shape([self.atoms.len(),nocc,nocc]).unwrap().slice(s![..,ind,..])));
                integral = integral - term_2;

            }
            // loop over gradient
            for n_at in 0..3*self.atoms.len(){
            ...
            let mut integral:Array2<f64> = 2.0 * qvv.view().into_shape([self.atoms.len(),nvirt,nvirt]).unwrap().slice(s![..,slice_ind,slice_ind]).dot(&g0.dot(&qoo))
                .into_shape([nocc,nocc]).unwrap();

            if self.gammafunction_lc.is_some(){
                let g0_lr:ArrayView2<f64> = self.data.gamma_lr();
                // integral (ik|il)
                let term_2:Array2<f64> = qvo.view().into_shape([self.atoms.len(),nvirt,nocc]).unwrap().slice(s![..,slice_ind,..]).t()
                    .dot(&g0_lr.dot(&qvo.view().into_shape([self.atoms.len(),nvirt,nocc]).unwrap().slice(s![..,slice_ind,..])));
                integral = integral - term_2;
            }
        ...
        let grad_h_mo: Array1<f64> = c_mo.dot(
            &(grad_h
                ...
                .unwrap()
                .dot(&c_mo)
                .into_shape([3 * self.atoms.len(), self.n_orbs()])
                .unwrap()
                .t()),
        );
        ...
        // // k = virt, l = occ
        // // let g0_lr:ArrayView2<f64> = self.data.gamma_lr();
        // let a_mat:Array2<f64> = 4.0* qoo.view().into_shape([self.atoms.len(),nocc,nocc]).unwrap()
        //     .slice(s![..,ind,ind]).dot(&g0.dot(&qvo)).into_shape([nvirt,nocc]).unwrap();
        //     // - qov.view().into_shape([self.atoms.len(),nocc,nvirt]).unwrap().slice(s![..,ind,..]).t()
        //     // .dot(&g0_lr.dot(&qoo.view().into_shape([self.atoms.len(),nocc,nocc]).unwrap().slice(s![..,ind,..])))
        //     // - qoo.view().into_shape([self.atoms.len(),nocc,nocc]).unwrap().slice(s![..,ind,..]).t()
        //     // .dot(&g0_lr.dot(&qov.view().into_shape([self.atoms.len(),nocc,nvirt]).unwrap().slice(s![..,ind,..]))).t();
        //
        // // calculate gradient term of the umatrix: sum_k sum_l U^a_kl A_ii,kl
        // let mut u_term:Array1<f64> = Array1::zeros(3*self.atoms.len());
        // for nat in 0..3*self.atoms.len(){
        ...
        // let mut grad_2:Array1<f64> = Array1::zeros(3*self.atoms.len());
        ...
        // let mut grad_omega_aowise:Array3<f64> = Array3::zeros([3*self.atoms.len(),self.n_orbs(),self.n_orbs()]);
        ...
        // }
        // let mut grad_3:Array1<f64> = Array1::zeros(3*self.atoms.len());
        //
        // for n_at in 0..3*self.atoms.len(){
        //     let mut term_2:Array1<f64> =  Array1::zeros([self.atoms.len()]);
        //     let mut mu:usize = 0;
        //     // P_mu,nu * DS_mu,nu
        ...
        // let mut grad_4:Array1<f64> = Array1::zeros(3*self.atoms.len());
        //
        // // calculate gradient of charges
        // let p_mat:ArrayView2<f64> = self.data.p();
        // let grad_dq: Array2<f64> = charges_derivatives_contribution(self.atoms.len(),self.n_orbs(),atoms,p_mat,s,grad_s.view());
        //
        // for n_at in 0..3*self.atoms.len(){
        //     // sum_C (g0_ac + g0_bc) * sum_mu on C,nu P_mu,nu * DS_mu,nu
