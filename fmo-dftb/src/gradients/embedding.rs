        ...
        self.pairs
            ...
            .for_each(|(pair, mut gradient)| {
                ...
                let delta_dq_f = delta_dq
                    .broadcast([3, pair.atoms.len()])
                    .unwrap()
                    .reversed_axes()
                    .as_standard_layout()
                    .into_shape([3 * pair.atoms.len()])
                    .unwrap()
                    .to_owned();
                ...
                gradient.slice_mut(s![m_i.slice.grad]).add_assign(
                    &(&delta_dq_f.slice(s![..3 * m_i.atoms.len()])
                ...
                let mut esp_ij: Array1<f64> = Array1::zeros([pair.atoms.len()]);
                esp_ij.slice_mut(s![..m_i.atoms.len()]).assign(
                    &(&m_i.data.esp_q()
                        - &gamma
                ...
                gradient
                    ...
                    .add_assign(&gddq_esp.slice(s![..3 * m_i.atoms.len()]));
                ...
                let mut dg_ddq: Array1<f64> = grad_gamma_sparse
                    ...
                    .dot(&delta_dq.slice(s![..m_i.atoms.len()]));
                ...
                dg_ddq
                    .slice_mut(s![m_i.slice.grad])
                    .assign(&Array1::zeros([3 * m_i.atoms.len()]));
                dg_ddq
                    .slice_mut(s![m_j.slice.grad])
                    .assign(&Array1::zeros([3 * m_j.atoms.len()]));
                ...
                ddq_gamma
                    ...
                    .assign(&Array1::zeros([m_i.atoms.len()]));
                ddq_gamma
                    .slice_mut(s![m_j.slice.atom])
                    .assign(&Array1::zeros([m_j.atoms.len()]));
...
fn get_grad_delta_dq(pair: &Pair, m_i: &Monomer, m_j: &Monomer) -> Array1<f64> {
    ...
    grad_delta_dq_2d
        .slice_mut(s![..(3 * m_i.atoms.len()), ..m_i.atoms.len()])
    ...
    let grad_delta_dq_3d: Array3<f64> = grad_delta_dq_2d
        .into_shape([3, pair.atoms.len(), pair.atoms.len()])
        .unwrap();
