...
impl<'a> GroundStateGradient for Monomer<'a> {
    fn scc_gradient(&mut self, atoms: &[Atom]) -> Array1<f64> {
        ...
        let grad_gamma: Array2<f64> =
            gamma_gradients_atomwise(&self.gammafunction, &atoms, self.atoms.len())
        ...
        //
        // // 3rd part: 0.5 * Z * dS / dR * sum_K sum_c_in_K (gamma_ac + gamma_bc) * dq_c
        // response += &(0.5 * &grad_s.dot(&esp_x_z));
        //
        // // Compute Z-vector charges
        // let z_s: Array1<f64> = (&z_vector.into_shape([self.n_orbs(), self.n_orbs()]).unwrap() * &s).sum_axis(Axis(1));
        // let mut zs_charges: Array1<f64> = Array1::zeros([self.atoms.len()]);
        // let mut mu: usize = 0;
        // for (atom, charge) in atoms.iter().zip(zs_charges.iter_mut()) {
        ...
        //     gamma_gradients_atomwise_2d(&self.gammafunction, atoms, atoms.len());
        // // 4th part
    ...
    fn get_grad_dq(&self, atoms: &[Atom], s: ArrayView2<f64>, grad_s: ArrayView3<f64>, p: ArrayView2<f64>) -> Array2<f64> {
        ...
        let mut grad_dq: Array2<f64> = Array2::zeros([f, self.atoms.len()]);
        let mut mu: usize = 0;
