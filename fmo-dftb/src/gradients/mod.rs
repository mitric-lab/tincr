...
impl<'a> SuperSystem<'a> {
    ...
    fn monomer_gradients(&mut self) -> Array1<f64> {
        ...
        //         .data
        //         .grad_dq()
        //         .into_shape([3, mol.atoms.len(), mol.atoms.len()])
        //         .unwrap();
        //
        //     grad_dq
    ...
    fn pair_gradients(&mut self, monomer_gradient: ArrayView1<f64>) -> Array1<f64> {
        ...
        //         &(&pair_grad.slice(s![0..(3*m_i.atoms.len())]) - &monomer_gradient.slice(s![m_i.slice.grad])),
        //     );
        //     gradient.slice_mut(s![m_j.slice.grad]).add_assign(
        //         &(&pair_grad.slice(s![(3*m_i.atoms.len())..]) - &monomer_gradient.slice(s![m_j.slice.grad])),
        //     );
        // }
