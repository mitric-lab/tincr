...
impl<'a> Pair<'a> {
    pub fn prepare_scc(&mut self, atoms: &[Atom], m1: &Monomer, m2: &Monomer) {
        ...
        let gamma_ab: Array2<f64> = gamma_atomwise_ab(
            &self.gammafunction,
            &atoms[0..m1.atoms.len()],
            &atoms[m1.atoms.len()..],
            m1.atoms.len(),
            m2.atoms.len(),
        );
        let mut gamma: Array2<f64> = Array2::zeros([self.atoms.len(), self.atoms.len()]);

        gamma
            .slice_mut(s![0..a, 0..a])
            .assign(&m1.data.gamma());
        gamma
            .slice_mut(s![a.., a..])
            .assign(&m2.data.gamma());
    ...
    pub fn run_scc(&mut self, atoms: &[Atom], config: SccConfig) {
        ...
        // Delta dq
        let mut dq: Array1<f64> = self.data.dq().to_owned();
        ...
        let mut last_energy: f64 = self.data.last_energy();
        let f: &[f64] = self.data.occupation();
        let v: ArrayView2<f64> = self.data.v();
        ...
        'scf_loop: for iter in 0..max_iter {
            ...
            if converged {
                let e_rep: f64 = get_repulsive_energy(&atoms, self.atoms.len(), &self.vrep);
                self.data.set_last_energy(scf_energy + e_rep);
                self.data.set_p(p);
        ...
        //self.data.clear();
        self.data.set_delta_dq(&dq - &self.data.dq());
        self.data.set_dq(dq);
    }
}