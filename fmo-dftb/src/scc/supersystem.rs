...
impl<'a> SuperSystem<'a> {
    ...
    pub fn embedding_energy(&self) -> f64 {
        ...
        for pair in self.pairs.iter() {
            ...
            embedding += esp_q_i.dot(&ddq.slice(s![..m_i.atoms.len()]));
            // The embedding energy for Monomer J in the pair is computed.
            embedding += esp_q_j.dot(&ddq.slice(s![m_i.atoms.len()..]));
        }
        ...
        //         &esp_q_j - &gamma.slice(s![m_j.slice.atom, m_i.slice.atom]).dot(&dq_i);
        //     // The embedding energy for Monomer I in the pair is computed.
        //     let mut embedd:f64 = esp_q_i.dot(&ddq.slice(s![..m_i.atoms.len()]));
        //     // The embedding energy for Monomer J in the pair is computed.
        //     embedd += esp_q_j.dot(&ddq.slice(s![m_i.atoms.len()..]));
        //     embedd
        // }).collect();
