...
/// of a gaussian.
pub fn gaussian_decay(unique_atoms: AtomSlice) -> HashMap<u8, f64> {
    ...
    for (number, hubbard) in soa_zip!(unique_atoms, [number, hubbard]){
        sigmas.insert(*number, 1.0 / (hubbard * PI_SQRT));
    }
    return sigmas;
}
...
pub fn gamma_atomwise(
    ...
    for (i, (xyz_i, number_i)) in soa_zip!(atoms, [xyz, number]).enumerate() {
        for (j, (xyz_j, number_j)) in soa_zip!(atoms, [xyz, number]).enumerate() {
            if i == j {
                g0[[i, j]] = gamma_func.eval_limit0(*number_i);
            } else if i < j {
                g0[[i, j]] = gamma_func.eval((xyz_i-xyz_j).norm(), *number_i, *number_j);
            } else {
                g0[[i, j]] = g0[[j, i]];
            }
        }
...
pub fn gamma_ao_wise(
    ...
    let g0: Array2<f64> = gamma_atomwise(gamma_func, atoms);
